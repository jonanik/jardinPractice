			...
			<div id="container">
				...
				<div id="outbox">
					...
					<div id="contents">
								...
								<table summary="NO, 제목, 등록일, 조회수 순으로 공지사항을 조회 하실수 있습니다."
									...
									<tbody>
										<c:forEach var="notice" items="${noticeList}">
											<tr>
												<td class="tnone">${notice.no}</td>
												<td class="left"><a href="noticeView?no=${notice.no }">${notice.title}</a> <img src="images/ico/ico_new.gif" alt="NEW" /></td>
												<td><fmt:formatDate value="${notice.upload_date }" pattern="yyyy-MM-dd"/></td>
												<td class="tnone right">${notice.hit}</td>
											</tr>
										</c:forEach>
									</tbody>
								</table>
						...
						<a href="noticeList?curPage=1&option=${sv.option }&search=${sv.search}" class="n"><img src="images/btn/btn_pre2.gif" alt="처음으로"/></a>
						<c:if test="${pDto.getPrev_page()}">
						
           				 <a href="noticeList?curPage=${pDto.getStart_page()-1}&option=${sv.option }&search=${sv.search}" class="pre"><img src="images/btn/btn_pre1.gif" alt="앞페이지로"/></a>
         					</c:if>
         					
						 <c:forEach begin="${pDto.getStart_page()}" end="${pDto.getEnd_page()}" step="1" var="index">
            				<c:if test="${pDto.getCurPage() eq index}">
               				<a  href="noticeList?curPage=${index}&option=${sv.option }&search=${sv.search}" style="color:#f7703c; border-color:#f7703c;">${index}</a>
            				</c:if>
            				<c:if test="${pDto.getCurPage() ne index}">
              				 <a href="noticeList?curPage=${index}&option=${sv.option }&search=${sv.search}">${index}</a>
            				...
            				<a href="noticeList?curPage=${pDto.getEnd_page()+1}&option=${sv.option }&search=${sv.search}" class="next"><img src="images/btn/btn_next1.gif" alt="뒤페이지로"/></a>
         					</c:if>
         					
						
						<a href="noticeList?curPage=${pDto.getPage_cnt()}&option=${sv.option }&search=${sv.search}" class="n"><img src="images/btn/btn_next2.gif" alt="마지막페이지로"/></a>
					...
					<div class="searchWrap">
						<div class="search">
							<ul>
								...
								<li class="web"><button style="border:none; cursor:pointer;"><img src="images/btn/btn_search.gif" alt="검색" /></button></li>
